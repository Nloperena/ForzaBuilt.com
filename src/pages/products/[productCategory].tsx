...
const getProductCategoryImage = (category: string) => {
  // For now, return null since we're consolidating data sources
  // This can be enhanced later to get images from the product database
  return null;
};
                  ...
                  <div className="absolute inset-0 flex md:flex-col justify-between p-4 md:p-6 text-white">
                    ...
                    <div className="hidden md:flex items-start justify-between">
                      ...
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          const industry = product.industry?.[0]?.toLowerCase() || '';
                        ...
                        title={`Filter by ${product.industry?.[0] || ''} industry`}
                      >
                        {getIndustryLogo(product.industry?.[0] || '') ? (
                          <img 
                            src={getIndustryLogo(product.industry?.[0] || '')} 
                          ...
                          <span className="text-white font-bold text-lg">{product.industry?.[0]?.charAt(0)?.toUpperCase() || ''}</span>
                        )}
                      </button>
                    </div>
