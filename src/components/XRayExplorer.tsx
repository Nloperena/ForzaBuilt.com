...
const XRayExplorer: React.FC<XRayExplorerProps> = ({ 
  ...
  const hotspotEndProgress = 0.85; // Reduced to 85% to make sticky behavior end sooner
  
  const hotspotProgress = useTransform(scrollYProgress, [hotspotStartProgress, hotspotEndProgress], [0, 1]);
  const hotspotsOpacity = useTransform(scrollYProgress, [hotspotStartProgress, hotspotStartProgress + 0.1], [0, 1]);
  ...
  const activeHotspotIndex = useTransform(hotspotProgress, (value) => {
    ...
    // Add +2 to create extra delay steps after final hotspot (2x the original)
    ...
    // Clamp to valid hotspot range (last steps are delays, so show last hotspot)
  ...
  return (
    ...
    >
      {/* Sticky Container */}
      <div className="sticky top-0 w-full h-screen flex items-end justify-center pb-32 overflow-hidden bg-white">
        <div className="relative w-full max-w-7xl mx-auto px-4">
          
          {/* Image Container */}
          <motion.div
            className="relative w-full mx-auto max-h-[60vh]"
            style={{
            ...
            >
              ...
              {xrayComponent.hotspots.map((hotspot, index) => {
                ...
                return (
                  ...
                  >
                    <motion.div 
                      className="w-8 h-8 bg-blue-600 text-white rounded-full flex items-center justify-center text-sm font-bold shadow-lg"
          ...
          >
            <motion.div 
              className="bg-white/95 backdrop-blur-sm rounded-lg px-6 py-3 border border-gray-200 shadow-lg"
