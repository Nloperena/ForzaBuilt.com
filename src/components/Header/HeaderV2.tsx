...
const HoverDropdown: React.FC<{ items: MenuItem[]; widthClass?: string; variant?: 'default' | 'industries' }> = ({ items, widthClass = 'w-[760px]', variant = 'default' }) => {
  ...
  return (
    <div className={`absolute left-1/2 -translate-x-1/2 top-full mt-2 ${responsiveWidth} ${variant === 'default' ? 'rounded-t-lg' : 'rounded-t-lg'} bg-[#2c476e] text-white shadow-2xl border-x border-b border-white/10 border-t-0 overflow-hidden z-20`}> 
              {variant === 'industries' ? (
