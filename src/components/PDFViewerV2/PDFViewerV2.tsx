...
const PDFViewerV2: React.FC<PDFViewerV2Props> = ({
  ...
  useEffect(() => {
    const updateWidth = () => {
      ...
      const spine = 12;
      const gap = 16; // Gap between two page boxes
      const widthByWidth = (availableWidth - spine - gap) / 2;
  ...
  return (
    ...
    >
      ...
      {!isLoading && !error && (
        ...
        >
            ...
            <button
              ...
              disabled={currentPage === 1 ? numPages < 2 : currentPage + 2 > numPages}
              className="p-1.5 hover:bg-white/10 disabled:opacity-30 disabled:cursor-not-allowed rounded transition-colors"
              title="Next"
            >
              <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className="flex-1 flex flex-col overflow-hidden relative" style={{ minHeight: 0 }}>
        ...
        >
          {!isLoading && !error && (
            <div className="flex-1 overflow-hidden relative flex items-start justify-center" style={{ paddingTop: '16px', paddingLeft: '16px', paddingRight: '16px', paddingBottom: 0 }}>
              <PageSpread
