...
const GradientTest: React.FC = () => {
  ...
  return (
    <div className="p-8 space-y-8">
      <h1 className="text-3xl font-bold mb-8">Gradient Test - Debug</h1>
      
      {testIndustries.map((industry) => {
        const colors = getIndustryColors(industry);
        const gradient = getBackgroundGradientByIndustry(industry);
        
        console.log(`${industry} colors:`, colors);
        console.log(`${industry} gradient:`, gradient);
        ...
        return (
          <div key={industry} className="border rounded-lg p-6 space-y-4">
            ...
            <div>
              <h3 className="font-semibold mb-2">Raw Gradient String</h3>
              <p className="text-sm bg-gray-100 p-2 rounded">{gradient}</p>
            </div>
          </div>
        );
      })}
