#!/usr/bin/env node
/**
 * Extract chemistry information from TDS PDF files
 * Searches through the "FOR EMAIL" versions of TDS files to gather chemistry data
 */
...
// Chemistry definitions and keywords based on Forza's official chemistry guide
const chemistryKeywords = {
  'Acrylic (incl. PSA)': [
    'acrylic', 'acrylate', 'psa', 'pressure sensitive', 'adhesive', 'acrylic polymer', 'acrylic resin',
    'free radical polymerization', 'pressure sensitive adhesive', 'psa adhesive'
  ],
  'Epoxy': [
    'epoxy', 'epoxies', 'amine', 'hardener', 'catalyst', 'resin', 'thermosetting resin',
    'chemical crosslinking', 'covalent bonds', 'epoxy resin', 'amine hardener'
  ],
  'Modified Epoxies': [
...
const productTypeKeywords = {
  'Tape': ['tape', 'transfer', 'double sided', 'backing', 'carrier', 'adhesive tape'],
  'Adhesive': ['adhesive', 'bond', 'glue', 'cement'],
  'Sealant': ['sealant', 'seal', 'joint', 'gap', 'filler'],
  'Coating': ['coating', 'paint', 'finish', 'protective'],
  'Resin': ['resin', 'polymer', 'matrix', 'composite'],
  'Canister': ['canister', 'cartridge', 'tube', 'dispenser']
};
...
function analyzeChemistry(text, productId) {
  ...
  // Check for product types (including Tape as a special category)
  for (const [type, keywords] of Object.entries(productTypeKeywords)) {
    const matches = keywords.filter(keyword => textLower.includes(keyword));
    if (matches.length > 0) {
      results.productType = type;
      results.indicators.push(...matches);
    }
  }

  // Special handling for tapes - they can have different chemistries
  if (results.productType === 'Tape') {
    // Tapes can be Acrylic, Silicone, or other chemistries
    // The chemistry detection above should handle this
    console.log(`  📋 Tape detected: ${productId} - Chemistry: ${results.chemistry}`);
  }
...
async function processTdsFiles() {
  ...
  const chemistrySummary = {
    ...
    chemistryDefinitions: chemistryDefinitions
  };
...
// Install pdf.js-extract if not available
