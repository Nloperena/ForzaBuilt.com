...
async function scrapeProductDetails(product) {
  ...
  try {
    ...
    // Extract applications from various selectors
    const applicationSelectors = [
      '.applications',
      '.product-applications',
      '.elementor-widget-text-editor:contains("Applications")',
      '.entry-content:contains("Applications")'
    ];
    
    for (const selector of applicationSelectors) {
      const $section = $(selector);
      if ($section.length > 0) {
        $section.find('li, p').each((i, item) => {
          const text = $(item).text().trim();
          if (text && text.length > 10 && !details.applications.includes(text)) {
    ...
    const technicalSelectors = [
      ...
      '.elementor-widget-text-editor:contains("Technical")',
      '.entry-content:contains("Technical")'
    ];
    
    for (const selector of technicalSelectors) {
      const $section = $(selector);
      if ($section.length > 0) {
        $section.find('tr, p').each((i, item) => {
          const text = $(item).text().trim();
    ...
    const certSelectors = [
      ...
      '.elementor-widget-text-editor:contains("Certification")',
      '.entry-content:contains("Certification")'
    ];
    ...
    for (const selector of certSelectors) {
      ...
      if ($section.length > 0) {
        $section.find('li, p').each((i, item) => {
          const text = $(item).text().trim();
          if (text && text.length > 5 && !details.certifications.includes(text)) {
    ...
    // Extract benefits, usage, colors, sizing, cleanup
...
async function scrapeAllSealProducts() {
  ...
  // Save results
  const outputPath = path.join(__dirname, 'sealProductsScraped.json');
